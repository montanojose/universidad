...
// =====================================================
// envios_recibir.php
// Envíos a recibir por el cliente
// - acceso para CLIENTE y ADMIN
// - CLIENTE: ve solo los envíos donde es destinatario
...
$titulo_pagina = 'Mis Envíos a Recibir';
...
function obtenerDniClienteSesionRecibir(): string
{
...
function normalizarTextoEstadoRecibir(?string $valor): string
{
...
if ($rol_actual === 'CLIENTE') {
    ...
    $dni_cliente_sesion = obtenerDniClienteSesionRecibir();
...
// -----------------------------------------------------
// 3. CARGAR LISTADO DE ENVÍOS A RECIBIR
// -----------------------------------------------------
...
try {
    ...
    if ($dni_cliente_filtro !== '' && !($rol_actual === 'ADMIN' && $buscar_es_tracking)) {
        $sqlEnvios .= " AND e.dni_destinatario = :dni_destinatario ";
        $params[':dni_destinatario'] = $dni_cliente_filtro;
    }
    ...
    $mensaje = 'Ocurrió un error al cargar los envíos a recibir.';
    $tipo_mensaje = 'error';
}
...
if (!empty($envios)) {
    ...
    foreach ($envios as $envio) {
        ...
        $estado = normalizarTextoEstadoRecibir($envio['cod_estado_actual'] ?? $envio['nombre_estado_actual'] ?? '');

        if ((int) $envio['disponible_retiro'] === 1 && (int) $envio['retirado'] === 0) {
        ...
        if (
            ...
            (
                str_contains($estado, 'EN_TRANSITO') ||
                str_contains($estado, 'DESPACHADO') ||
                str_contains($estado, 'ASIGNADO') ||
                str_contains($estado, 'RECIBIDO') ||
                str_contains($estado, 'CLASIFICADO') ||
                str_contains($estado, 'SUCURSAL')
            )
        ) {
            $resumen['en_camino']++;
        }
...
<main class="app-content">
    ...
    <section class="page-header">
        <h1 class="page-title">Mis Envíos a Recibir</h1>
        <p class="page-subtitle">
            Consultá los envíos donde figurás como destinatario y revisá cuándo quedan disponibles para retiro.
        </p>
    </section>
    ...
    <section class="dashboard-grid" style="margin-bottom: 24px;">
        ...
        <article class="dashboard-card">
            <h3 style="margin-top: 0;">En camino</h3>
            <p style="font-size: 28px; margin: 0; font-weight: 700;">
                <?php echo htmlspecialchars((string) $resumen['en_camino']); ?>
            </p>
        </article>
    ...
    <section class="dashboard-card" style="margin-bottom: 24px;">
        ...
        <form method="GET" action="envios_recibir.php">
            ...
            <div class="dashboard-grid" style="grid-template-columns: <?php echo $rol_actual === 'ADMIN' ? '1fr 2fr 1fr 1fr' : '2fr 1fr 1fr'; ?>;">
                ...
                <div class="form-group">
                    <label for="buscar">Buscar por tracking, remitente u origen/destino</label>
                    <input
                        type="text"
                        id="buscar"
                        name="buscar"
                        ...
                        placeholder="Ej: TRK000001, Gómez, Mendoza..."
                    >
                </div>
                ...
                <div class="form-group" style="display: flex; align-items: flex-end; gap: 12px;">
                    ...
                    <a href="envios_recibir.php" class="btn-public-secondary">
                        Limpiar
                    </a>
                </div>
    ...
    <section class="dashboard-card">
        ...
        <h3 style="margin-top: 0; margin-bottom: 18px;">
            <?php echo $rol_actual === 'CLIENTE' ? 'Mis envíos a recibir' : 'Envíos a recibir'; ?>
        </h3>
            ...
            <table style="width: 100%; border-collapse: collapse; min-width: 1800px;">
                ...
                <tbody>
                    <?php if (empty($envios)): ?>
                        ...
                        <tr>
                            <td colspan="<?php echo $rol_actual === 'ADMIN' ? '14' : '13'; ?>" style="padding: 16px; text-align: center; color: var(--color-muted); font-style: italic;">
                    ...
                    <?php else: ?>
                        ...
                        <?php foreach ($envios as $envio): ?>
                            <tr>
                                ...
                                <td style="padding: 12px; border-bottom: 1px solid var(--color-border);">
                                    <?php echo ((int) $envio['disponible_retiro'] === 1 && (int) $envio['retirado'] === 0) ? 'Sí' : 'No'; ?>
                                </td>

                                <td style="padding: 12px; border-bottom: 1px solid var(--color-border);">
                                    <?php echo htmlspecialchars($envio['fecha_limite_retiro'] ?? ''); ?>
                                </td>
                                ...
                                <td style="padding: 12px; border-bottom: 1px solid var(--color-border); white-space: nowrap;">
                                    ...
                                    <a href="historial_envio.php?tracking=<?php echo urlencode($envio['nro_tracking']); ?>" class="btn-public-secondary" style="margin-right: 8px;">
                                        Ver historial
                                    </a>

                                    <?php if ((int) $envio['disponible_retiro'] === 1): ?>
                                        <a href="datos_retiro.php?tracking=<?php echo urlencode($envio['nro_tracking']); ?>" class="btn-public-secondary" style="margin-right: 8px;">
                                            Datos retiro
                                        </a>
                                    <?php endif; ?>

                                    <?php if ((int) $envio['retirado'] === 0): ?>
